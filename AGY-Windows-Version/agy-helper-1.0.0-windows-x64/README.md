...
- 🪟 **Windows Installation (Includes Proper Signing Package)**:
  1. Download and extract **`AgyHelper-Windows-x64.zip`**.
  2. Right-click **`Setup-AgyHelper.bat`** (or **`Install-Certificate.bat`**) and select **Run as administrator**.
  3. Click **Yes** at the Windows prompt to install the signing certificate into your trusted root store.
  4. Agy Helper will start automatically and is now recognized as a trusted application without SmartScreen warnings!
  - See [INSTALL-WINDOWS.md](INSTALL-WINDOWS.md) for full step-by-step instructions.
- 🍎 **macOS**: Download `AgyHelper-macOS.dmg`
